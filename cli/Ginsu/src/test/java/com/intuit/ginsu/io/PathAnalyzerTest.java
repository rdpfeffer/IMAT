...
public class PathAnalyzerTest extends BaseFunctionalTest{
	...
	public void beforeMethod() 
	{
		...
		try {
			this.tempFile = convertToURI(File.createTempFile("placeholder", ".txt"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	{
		URI toPath = convertToURI(this.getTestResourceAsFile("pathTest"+File.separator
				+"grandfather"+File.separator
				+"father"+File.separator
				+"child"+File.separator
				+PLACEHOLDER_FILE));
		URI fromPath = convertToURI(this.getTestResourceAsFile("pathTest"+File.separator
				+"grandfather"+File.separator
				+PLACEHOLDER_FILE));
