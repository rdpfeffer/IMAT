...
public class PropertyFileConfigurationServiceTest {
	...
	{
		//for an exception not to be thrown when loading app
		//properties, we just need one property to be set.
	...
	{
		mockAppResourceService = EasyMock.createMock(IApplicationResourceService.class);
		mockProjectResourceService = EasyMock.createMock(IProjectResourceService.class);
	}
	
	@Test
	public void testLoadConfigExpectingProjectWithProject() throws MisconfigurationException, ProjectConfigurationNotFoundException {
		//expect a call to get the config files
		EasyMock.expect(mockAppResourceService.getAppProperties(APP_CONFIG_FILE))
			.andReturn(appProperties);
		EasyMock.expect(mockProjectResourceService.getProjectProperties(PROJECT_CONFIG_FILE))
			.andReturn(projectProperties);
		...
		PropertyFileConfigurationService configService = new PropertyFileConfigurationService(
				...
				APP_CONFIG_FILE,
				EasyMock.createNiceMock(Logger.class));
	...
	public void testLoadConfigExpectingProjectWithoutProject() throws MisconfigurationException, ProjectConfigurationNotFoundException {
		//expect a call to get the config files
		EasyMock.expect(mockAppResourceService.getAppProperties(APP_CONFIG_FILE))
			.andReturn(appProperties);
		EasyMock.expect(mockProjectResourceService.getProjectProperties(PROJECT_CONFIG_FILE))
	...
	@Test
	public void testLoadConfigExpectingNoProjectWitProject() throws MisconfigurationException, ProjectConfigurationNotFoundException {
		//expect a call to get the config files
		EasyMock.expect(mockAppResourceService.getAppProperties(APP_CONFIG_FILE))
			.andReturn(appProperties);
		EasyMock.expect(mockProjectResourceService.getProjectProperties(PROJECT_CONFIG_FILE))
		...
		mockLogger.debug(EasyMock.contains("unable to load any properties from the project configuration file"));
		...
		EasyMock.verify(mockProjectResourceService);
		assert AppContext.INSTANCE.getProperty("foo") == "bar" : "Application properties were not set.";
		assert AppContext.INSTANCE.getProperty("monkey").isEmpty() : "Project property was not empty: " + AppContext.INSTANCE.getProperty("monkey");
	}

	@Test
	public void testLoadConfigExpectingNoProjectWithoutProject() throws MisconfigurationException, ProjectConfigurationNotFoundException {
		//expect a call to get the config files
		EasyMock.expect(mockAppResourceService.getAppProperties(APP_CONFIG_FILE))
			.andReturn(appProperties);
		EasyMock.expect(mockProjectResourceService.getProjectProperties(PROJECT_CONFIG_FILE))
	...
	public void testLoadConfigWithoutAppConfg() throws MisconfigurationException, ProjectConfigurationNotFoundException {
		EasyMock.expect(mockAppResourceService.getAppProperties(APP_CONFIG_FILE))
			.andReturn(new Properties());
		EasyMock.expect(mockProjectResourceService.getProjectProperties(PROJECT_CONFIG_FILE))
			.andReturn(projectProperties);
